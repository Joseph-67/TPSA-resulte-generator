...
# load main template
wb = load_workbook("templates/2024_2025/second_term/SECONDARY TEMPLATE (1) (1).xlsx")
# create a worksheet
ws = wb.active
print(ws["B3"].value)

list_of_students = ST.ss3art

# names = [ ]
no_of_students = len(list_of_students)
count = 1
for x in list_of_students:
    sheet_to_copy = wb['SS-ART-MAIN-TEMPLATE']
    ...
    new_sheet["B5"].value = "S.S.S THREE"
    new_sheet["F5"].value = "SCIENCE"
    new_sheet["I5"].value = no_of_students
    ...
    data = Reference(new_sheet, min_col=5, min_row=9, max_row=22)
    cats = Reference(new_sheet, min_col=1, min_row=10, max_row=22)
    chart1.add_data(data, titles_from_data=True)
    chart1.set_categories(cats)
    # chart1.shape = 4
    new_sheet.add_chart(chart1, "A33")
    app_name = 'SSS3ART'
    ...
    new_sheet["B1"].font = Font(name="Times New Roman", sz= 64, b=True, shadow=True)
    new_sheet["B2"].value = "SECOND TERM 2024/2025 SESSION ACADEMIC REPORT"
    new_sheet["B2"].font = Font(name="Times New Roman", sz= 26, b=True, shadow=True)
    # end heading
